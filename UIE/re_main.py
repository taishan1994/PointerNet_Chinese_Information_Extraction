...
from UIE.config import ReArgs
from UIE.re_data_loader import ReDataset, ReCollate
...
class RePipeline:
    ...
    def rel_eval_forward(self, 
        ...
        for eval_step, batch_data in enumerate(data_loader):
            ...
            output = self.model(re_rel_input_ids=batch_data["re_rel_input_ids"],
                        re_rel_token_type_ids=batch_data["re_rel_token_type_ids"],
                        re_rel_attention_mask=batch_data["re_rel_attention_mask"],
                        re_rel_labels=batch_data["re_rel_labels"],
                        )
            tmp_rel_logits = output["re_output"]["rel_logits"].detach().cpu()
            tmp_rel_labels = batch_data["re_rel_labels"].detach().cpu()
            

            if rel_labels is None:
                rel_logits = tmp_rel_logits
                rel_labels = tmp_rel_labels
            else:
                rel_logits = np.append(rel_logits, tmp_rel_logits, axis=0)
                rel_labels = np.append(rel_labels, tmp_rel_labels, axis=0)
        ...
        metrics = self.get_rel_metrics(bj_outputs, 
                        label=label,
                        id2label=id2label,
                        return_report=return_report)
        metrics["rel_metrics"] = metrics

        return metrics

    def get_rel_metrics(self, 
              rel_outputs, 
              label,
              id2label,
              return_report=False):
        metrics = self.get_rel_metrics_helper(rel_outputs, id2label, return_report)

        return metrics

    def get_rel_metrics_helper(self, outputs, id2label, return_report):
        rel_logits = outputs["rel_logits"]
        rel_logits = np.where(sigmoid(rel_logits) > 0.5, 1, 0)
        rel_labels = outputs["rel_labels"]
        precsion = precision_score(rel_labels, rel_logits, average="micro")
        recall = recall_score(rel_labels, rel_logits, average="micro")
        f1 = f1_score(rel_labels, rel_logits, average="micro")
        metrics = {
          "precision": precsion,
          "recall": recall,
          "f1": f1,
        ...
        if return_report:
          report = cr(rel_labels, rel_logits, target_names=list(id2label.values()))
    ...
    def train(self, dev=True):
        train_dataset = ReDataset(file_path=self.args.train_path,
                      tokenizer=self.args.tokenizer,
                      max_len=self.args.max_seq_len,
                      relation_label=self.args.relation_label,
                      entity_label=self.args.entity_label)
        collate = ReCollate(max_len=self.args.max_seq_len, tokenizer=self.args.tokenizer, tasks=self.args.tasks)
        ...
        dev_callback = None
        if dev:
            dev_dataset = ReDataset(file_path=self.args.dev_path,
                        tokenizer=self.args.tokenizer,
                        max_len=self.args.max_seq_len,
                        relation_label=self.args.relation_label,
                        entity_label=self.args.entity_label)
        ...
        for epoch in range(1, self.args.train_epoch + 1):
            for step, batch_data in enumerate(train_loader):
                ...
                if "ner" in self.args.tasks:
                  ...
                  loss = output["re_output"]["sbj_loss"]
                ...
                elif "rel" in self.args.tasks:
                  output = self.model(re_rel_input_ids=batch_data["re_rel_input_ids"],
                            re_rel_token_type_ids=batch_data["re_rel_token_type_ids"],
                            re_rel_attention_mask=batch_data["re_rel_attention_mask"],
                            re_rel_labels=batch_data["re_rel_labels"],
                            )
                  loss = output["re_output"]["rel_loss"]
                ...
                if dev and global_step % eval_step == 0:
                  if "ner" in self.args.tasks:
                  ...
                  elif "sbj" in self.args.tasks or "obj" in self.args.tasks:
                    if "sbj" in self.args.tasks:
                      label = ["主体"]
                      id2label = {0:"主体"}
                    else:
                    ...
                    metrics = self.rel_eval_forward(dev_loader, self.args.relation_label, self.args.rel_id2label)
                    rel_metrics = metrics["rel_metrics"]
                    print('【eval】 precision={:.4f} recall={:.4f} f1_score={:.4f}'.format(rel_metrics["precision"],
                                                          rel_metrics["recall"],
                                                          rel_metrics["f1"]))
                    if rel_metrics["f1"] > best_f1:
                        best_f1 = rel_metrics["f1"]
                        print("【best_f1】：{}".format(best_f1))
                        self.save_model() 
    ...
    def test(self):
        test_dataset = ReDataset(file_path=self.args.test_path,
                      tokenizer=self.args.tokenizer,
                      max_len=self.args.max_seq_len,
                      relation_label=self.args.relation_label,
                      entity_label=self.args.entity_label)
        ...
        with torch.no_grad():
          if "ner" in self.args.tasks:
            ...
            metrics = self.rel_eval_forward(test_loader, self.args.relation_label, self.args.rel_id2label, return_report=True)
            rel_metrics = metrics["rel_metrics"]
            print('【test】 precision={:.4f} recall={:.4f} f1_score={:.4f}'.format(rel_metrics["precision"],
                                                            rel_metrics["recall"],
                                                            rel_metrics["f1"]))
    ...
    def predict(self, textb, texta=None):
        ...
        with torch.no_grad():
            if "ner" in self.args.tasks:
              ...
              token_type_ids = torch.from_numpy(np.array(encode_dict['token_type_ids'])).unsqueeze(0).to(self.args.device)
              output = self.model(
                re_sbj_input_ids=token_ids,
                re_sbj_token_type_ids=token_type_ids,
                re_sbj_attention_mask=attention_masks,
              )
              start_logits = output["re_output"]["sbj_start_logits"].detach().cpu()
              end_logits = output["re_output"]["sbj_end_logits"].detach().cpu()
              length = sum(attention_masks.detach().cpu()[0])
              pred_entities = bj_decode(start_logits, end_logits, length, {0:"主体"})
              values = pred_entities["主体"]
              subjects = []
              for v in values:
                start = v[0]
                end = v[1]
                subjects.append("".join(tokens_b[start:end]))
              ...
              pred_entities = bj_decode(start_logits, end_logits, length, {0:"客体"})
              values = pred_entities["客体"]
              objects = []
              for v in values:
                start = v[0]
                end = v[1]
              ...
              texta = texta.split("#;#")
              tokens = [i for i in texta[0]] + ['[SEP]'] + [i for i in texta[1]] + ['[SEP]'] + [i for i in textb]
              attention_mask = [1] * len(tokens)
              token_type_ids = [0] * self.args.max_seq_len
              if len(tokens) > self.args.max_seq_len - 2:
                tokens = tokens[:self.args.max_seq_len-2]
              ...
              token_type_ids = torch.from_numpy(np.array(token_type_ids)).unsqueeze(0).to(self.args.device)
              output = self.model(
                re_rel_input_ids=token_ids,
                re_rel_token_type_ids=token_type_ids,
                re_rel_attention_mask=attention_mask,
              )
              rel_logits = output["re_output"]["rel_logits"].detach().cpu()
              rel_logits = np.where(sigmoid(rel_logits)>0.5, 1, 0).tolist()[0]
              rels = []
              for i,t in enumerate(rel_logits):
                if t == 1:
                 rels.append(self.args.rel_id2label[i])
              return rels
...
if __name__ == '__main__':
    ...
    if "ner" in args.tasks:
      raw_text = "《父老乡亲》是由是由由中国人民解放军海政文工团创作的军旅歌曲，石顺义作词，王锡仁作曲，范琳琳演唱"
      print(raw_text)
      print(re_pipeline.predict(raw_text))
    elif "sbj" in args.tasks:
      raw_text = "《父老乡亲》是由是由由中国人民解放军海政文工团创作的军旅歌曲，石顺义作词，王锡仁作曲，范琳琳演唱"
      raw_text = "《邪少王兵》是冰火未央写的网络小说连载于旗峰天下"
      print(raw_text)
      print(re_pipeline.predict(raw_text))
    elif "obj" in args.tasks:
      textb = "《父老乡亲》是由是由由中国人民解放军海政文工团创作的军旅歌曲，石顺义作词，王锡仁作曲，范琳琳演唱"
      texta = "王锡仁"
      textb = "《邪少王兵》是冰火未央写的网络小说连载于旗峰天下"
      texta = "邪少王兵"
      print(textb)
      print(texta)
      print(re_pipeline.predict(textb, texta))
    elif "rel" in args.tasks:
      textb = "《父老乡亲》是由是由由中国人民解放军海政文工团创作的军旅歌曲，石顺义作词，王锡仁作曲，范琳琳演唱"
      texta = "石顺义#;#父老乡亲"
      print(textb)
      print(texta)
